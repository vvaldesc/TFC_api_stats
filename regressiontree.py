...
def regressiontree(historical, request, test = False, loadTrained = False):
    ...
    X = historical_data.drop(["delay", "created_at"], axis=1)  # Exclude non-numeric and target columns
    ...
    request_data = pd.DataFrame([request]).drop(["created_at"], axis=1)  # Adjust based on actual columns
